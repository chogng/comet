---
description: Use when asked to consume workbench tree widgets in Comet Studio.
---

# Workbench Tree Widgets Overview

**Location**: `src/cs/platform/list/browser/listService.ts`
**Type**: Platform Services
**Layer**: Platform

## Purpose

The Workbench Tree Widgets provide high-level, workbench-integrated tree components that extend the base tree implementations with Comet Studio-specific functionality like context menus, keyboard navigation, theming, accessibility, and dependency injection integration. These widgets serve as the primary tree components used throughout the Comet Studio workbench for file explorers, debug views, search results, and other hierarchical data presentations.

## Scope

### Included Functionality
- **Context Integration**: Automatic context key management, focus handling, and Comet Studio theme integration
...
- **Dependency Injection**: Full integration with Comet Studio's service container for automatic service injection
...
- **`src/cs/platform/list/browser/listService.ts`**: Contains all workbench tree widget implementations, shared workbench functionality (`WorkbenchTreeInternals`), and configuration utilities
	- `src/cs/platform/list/browser/test/listService.test.ts`: Unit tests for workbench trees
- **`src/cs/base/browser/ui/tree/objectTree.ts`**: Base implementation for static trees and compressible trees
	- `src/cs/base/test/browser/ui/tree/objectTree.test.ts`: Base tree tests
- **`src/cs/base/browser/ui/tree/asyncDataTree.ts`**: Base implementation for async trees with lazy loading support
	- `src/cs/base/test/browser/ui/tree/asyncDataTree.test.ts`: Async tree tests
- **`src/cs/base/browser/ui/tree/dataTree.ts`**: Base implementation for data-driven trees with explicit data sources
	- `src/cs/base/test/browser/ui/tree/dataTree.test.ts`: Data tree tests
- **`src/cs/base/browser/ui/tree/abstractTree.ts`**: Base tree foundation
- **`src/cs/base/browser/ui/tree/tree.ts`**: Core interfaces and types
