...
import { createPdfiumLiteParsedDocument } from 'ls/editor/browser/pdf/pdfiumLiteParsedDocument';
...
test('createPdfiumLiteParsedDocument emits versioned passages with evidence pointers', () => {
  const artifact = createPdfiumLiteParsedDocument({
    ...
    pages: [createPageInfo('Literature Studio PDF smoke')],
  });

  assert.equal(artifact.schemaVersion, 'ls.parsedDocument.v1');
  ...
  assert.equal(artifact.passages[0]?.text, 'Literature Studio PDF smoke');
  assert.deepEqual(artifact.passages[0]?.evidence, {
    ...
    quote: 'Literature Studio PDF smoke',
  });
});
