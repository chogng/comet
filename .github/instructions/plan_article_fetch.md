...
Workbench session 不保存文章领域状态或文章 View 状态。当前 `activeJournalId` 和 `activeSourceId` 只属于 ChatInputPart context view；关闭该 View 即释放。未来若新增独立 Article Detail View，其 `activeArticleId` 由该 View 自己持有。Tree 的 row selection、focus、展开 Group 和滚动位置同样属于对应 View。Catalog 中可见的 Source、Page、Group、ListItem 和 Detail 均在渲染时查询，不写入工作台会话。

文章 checkbox 的勾选状态只有一个 Owner：`ChatService.checkedArticleIds`。当前 checkbox 位于 ChatListRenderer / ChatListWidget 渲染的聊天文章列表中，不属于 Tree selection 或 focus。Workbench 不保存 `checkedArticleIds`；下载和导出命令接收 ChatService 在命令触发时生成的只读 ID Snapshot，也不取得勾选状态所有权。
...
ChatListWidget 直接注入 IChatService 并注册 `subscribe()`。渲染 checkbox 时按 ArticleId 查询 ChatService，`Checkbox.onChange` 把组件的 checked 值传给 `setArticleChecked()`；ChatService 更新 Snapshot 并通知订阅者后，ChatListWidget 重新渲染当前消息。`isArticleSelected(href)`、`toggleArticleSelected(href)` 以及由 Workbench / SessionChatView 透传的 checkbox Props 直接迁移并删除，不保留 URL 兼容入口或专用 checkbox 事件。

`checkedArticleIds` 是无重复、按勾选先后排序的只读数组。`setArticleChecked(id, true)` 只在缺失时追加，`setArticleChecked(id, false)` 移除该 ID；状态未变化时不发布订阅通知。该顺序可作为一次命令的输入顺序，但不得写入 ArticleRecord、ArticleDetail 或 ArticleListItem。
...
ChatService
    不订阅 Fetch 事件维护文章副本
    只保存消息中的 ArticleId 引用和 checkedArticleIds
    ask() 时通过 IFetchService 查询 ArticleRecord / ArticleDetail
~~~
...
4. ChatService 不订阅 Fetch 事件主动同步勾选状态；ask、下载或导出开始前解析 checkedArticleIds，原子移除无法解析的 ID、发布一次 Chat Snapshot 变化并向用户报告缺失项。
...
1. 工作台会话删除 articles、checked/selected keys 和 active Journal/Source/Article；当前导航 ID 归具体 View 局部持有，checkedArticleIds 唯一归 ChatService。
...
6. ChatService Snapshot 只保存消息中的 ArticleId 引用和 checkedArticleIds；ChatListWidget 复用 `IChatService.subscribe()`，不新增 checkbox 专用事件。
...
6. ChatService 不订阅 Fetch 事件维护文章副本，只保存消息中的 ArticleId 引用和 checkedArticleIds。
7. ChatListWidget 直接订阅现有 `IChatService.subscribe()`，不增加 checkbox 专用事件。
8. Checkbox 通过 ArticleId 读写 ChatService；不存在 href 身份、DOM 反解析或 Tree selection 映射。
...
20. checkedArticleIds 只有 ChatService 一个 Owner，Workbench、下载和导出不保存副本。
...
40. ChatService 不订阅 Fetch 事件维护文章副本，只保存消息中的 ArticleId 引用和 checkedArticleIds，并在 ask() 时查询 IFetchService。
41. ChatListWidget 直接复用 `IChatService.subscribe()`，不存在 checkbox 专用事件或 checkbox Props 透传。
42. 当前 Chat checkbox 直接以 ArticleId 读写 ChatService，不通过 href、DOM 反解析或 Tree selection/focus 建模。
...
54. checkedArticleIds 只有 ChatService 一个 Owner；下载和导出只接收命令时的只读 ID Snapshot。
...
66. checkedArticleIds 无重复、保持勾选顺序，重复设置相同状态不产生 ChatService 通知。
