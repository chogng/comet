...
export interface IInstalledDomTestEnvironment {
...
const globalKeys: readonly GlobalKey[] = Object.freeze([
	'window',
	'document',
	'navigator',
	'HTMLElement',
...
export function installDomTestEnvironment(): IInstalledDomTestEnvironment {
	const dom = new JSDOM('<!doctype html><html><body></body></html>', {
		url: 'http://localhost/',
		pretendToBeVisual: true,
	});
	const previousDescriptors =
		new Map<GlobalKey, PropertyDescriptor | undefined>();
	...
	elementPrototype.getBoundingClientRect = () => createDomRect();
	elementPrototype.getClientRects = () => createDomRectList();
	rangePrototype.getBoundingClientRect = () => createDomRect();
	rangePrototype.getClientRects = () => createDomRectList();

	return {
		cleanup() {
			const currentDocument =
				globalTarget['document'] as Document | undefined;
