...
test('JSONSchemaRegistry registers schemas and fires change events', () => {
  ...
  const subscription = registry.onDidChangeSchema((event) => {
	...
	const disposable = registry.registerSchema('cs://schemas/example', {
    type: 'object',
    properties: {
      title: { type: 'string' },
    },
	...
	assert.equal(registry.getSchema('cs://schemas/example')?.schema.type, 'object');
	assert.deepEqual(changedUris, ['cs://schemas/example']);

  disposable.dispose();
	assert.equal(registry.getSchema('cs://schemas/example'), undefined);
