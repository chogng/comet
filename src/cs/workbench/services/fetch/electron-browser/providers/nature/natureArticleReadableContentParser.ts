...
export const natureArticleReadableBodySelector = 'main .c-article-body';
...
export function parseNatureArticleReadableContent(
	document: Document,
	base: URI,
): ParsedArticleReadableContent {
	const body = document.querySelector(natureArticleReadableBodySelector);
	if (!body) {
		throw new Error(`Nature article "${base.toString(true)}" does not contain its readable body.`);
	}
	return {
		url: base,
		title: title(document, base),
		text: extractArticleReadableText(body, `Nature article "${base.toString(true)}"`),
	};
}
