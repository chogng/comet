...
import type {
  EditorGroupModel,
  EditorGroupTabItem,
} from 'cs/workbench/browser/parts/editor/editorGroupModel';
...
let createEditorGroupModel: typeof import('cs/workbench/browser/parts/editor/editorGroupModel').createEditorGroupModel;
...
function getDropIndicatorLeft(element: HTMLElement) {
  return element.style.getPropertyValue('--editor-tab-drop-indicator-left');
}
...
before(async () => {
  ...
  ({ TabsTitleControl } = await import('cs/workbench/browser/parts/editor/tabsTitleControl'));
  ({ createEditorGroupModel } = await import('cs/workbench/browser/parts/editor/editorGroupModel'));
});
...
test('TabsTitleControl reorders tabs by drag and drop', () => {
  const reorderCalls: Array<[string, number]> = [];
  const control = new TabsTitleControl({
    group: createGroupModel('draft-a', [
      createTabItem({
        id: 'draft-a',
    ...
    onActivateTab: () => {},
    onReorderTab: (tabId, targetSlotIndex) => {
...
test('TabsTitleControl clears focused tab controls after drag ends without a reorder', () => {
  const control = new TabsTitleControl({
    group: createGroupModel('draft-a', [
      createTabItem({
        id: 'draft-a',
...
test('TabsTitleControl keeps the insertion indicator visible when host drag events jitter around a gap', () => {
  const control = new TabsTitleControl({
    group: createGroupModel('pdf-c', [
      createTabItem({
        id: 'draft-a',
...
test('TabsTitleControl keeps the same drop side for small cursor moves near a tab midpoint', () => {
  ...
  assert.equal(getDropIndicatorLeft(rootElement), '100px');
...
test('TabsTitleControl keeps the first drop indicator fully visible at the leading edge', () => {
  const control = new TabsTitleControl({
    group: createGroupModel('pdf-c', [
      createTabItem({
        id: 'draft-a',
  ...
  const dataTransfer = createDataTransferStub();
  dispatchDragEvent(pdfButton, 'dragstart', { dataTransfer });
  dispatchDragEvent(container, 'dragover', {
    clientX: -8,
    dataTransfer,
  });
...
test('TabsTitleControl renders its DOM context menu below the cursor for available tabs near the top edge', async () => {
  ...
  HTMLElement.prototype.getBoundingClientRect = function () {
    ...
    if (this.classList.contains('cs-menu')) {
      return {
        x: 0,
        y: 0,
        width: 160,
  ...
  try {
    ...
    const menu = document.body.querySelector('.cs-context-view .cs-menu');
