...
import { buildDocxBuffer, escapeXml, type DocxContentTypeDefault, type DocxContentTypeOverride, type DocxRelationship, type ZipEntry } from 'cs/code/electron-main/document/docxPackage';
import { defaultDocxExportConfig } from 'cs/code/electron-main/document/docxConfig';
import { resolveDocxExportCopy } from 'cs/code/electron-main/document/docxCopy';
...
function buildFigureBookmarkName(value: string | null | undefined) {
  ...
  // Word 书签名不能直接吃任意 figureId，这里保留可读前缀，再拼稳�?hash 避免冲突�?
...
function buildTextRunXml(run: EditorDocxRun) {
  ...
  if (run.backgroundColor) {
    // Word �?inline run 不支持真正的 pill 圆角，这里用 shading + border 做最接近的近似�?
    runProperties.push(
...
function buildListMarkerParagraphXml(
  ...
  return [
    '<w:p>',
    // 列表里的 figure 仍然先输出一个编号段落，后面�?table 再用同样缩进贴齐正文区域�?
...
function buildFigureTableXml(
  ...
  return [
    shouldEmitListMarker
      ...
      : '',
    // 用单列表格包住图片和 caption，导出后�?Word 里最接近编辑器里�?figure 卡片�?
    '<w:tbl>',
    '<w:tblPr>',
...
function buildDocumentXml(
  ...
  for (let index = 0; index < blocks.length; index += 1) {
    ...
    // collectBlocks 会把 figure 展平�?image + optional figcaption，这里再把它们重新合并成一�?Word 容器�?
    bodyParts.push(
      buildFigureTableXml(
        block,
        image,
