...
function reduceMoveNode(
	...
	if (sameParent) {
		...
		const sourceRemovalOrdinalMutation = Object.freeze({
			parentNodeId: sourceParent.id,
			kind: 'delete',
			childIndex: sourceLocation.childIndex,
		}) satisfies ParentOrdinalMutationAtParent;
...
type ParentOrdinalMutationAtParent = ParentOrdinalMutation & {
	readonly parentNodeId: NodeId;
};
...
class UpdatedDocumentIndex implements DocumentIndex {
	...
	constructor(constructionToken: object, options: {
		...
		if (constructionToken !== updatedDocumentIndexConstructionToken) {
			throw new TypeError(
				'Updated document indexes can only be constructed by the reducer.',
			);
		}
...
function applyParentOrdinalMutation(
	childIndex: number,
...
function createUpdatedDocumentIndex(
	...
	const editedParentNodeIds = new Set<NodeId>();
	for (const edit of update.parentOrdinalEdits ?? []) {
		const parentNodeId = edit.parentNodeId;
		if (editedParentNodeIds.has(parentNodeId)) {
			throw new Error(
				'A document index update may append at most one edit per parent.',
			);
		}
		editedParentNodeIds.add(parentNodeId);
...
function replaceNodeAtExistingPath(
	...
	let current = replacement;
	for (
		let pathIndex = path.nodes.length - 2;
		pathIndex >= 0;
		pathIndex -= 1
	) {
		const parent = path.nodes[pathIndex];
		const childIndex = path.childIndexes[pathIndex];
		if (
			parent === undefined
