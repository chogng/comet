...
import { Storage } from 'ls/base/parts/storage/common/storage';
...
async function withStorageFile<T>(run: (storageFile: string) => Promise<T>) {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'ls-sqlite-storage-'));
  try {
...
test('SQLiteStorageDatabase persists storage values across instances', async () => {
  await withStorageFile(async (storageFile) => {
    ...
    await storage.set('title', 'Literature Studio');
    ...
    assert.equal(restored.get('title'), 'Literature Studio');
