...
import { assertArticleContexts } from 'cs/workbench/services/llm/mainAgentPayload';
...
export async function answerQuestionFromArticles(
	...
	if (payload.articleContexts !== undefined && !Array.isArray(payload.articleContexts)) {
		throw new TypeError('RAG articleContexts must be an array when provided.');
	}
	const articleContexts = payload.articleContexts ?? [];
	assertArticleContexts(articleContexts);
	const articles = articleContexts;
